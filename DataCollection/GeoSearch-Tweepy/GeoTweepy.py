...
class StdOutListener(StreamListener):
    ...
    def on_status(self, status):
        ...
        try:
            ...
            # Place the coordinates values into a list 'XY'
            ...
            #print "Y: ", XY[1]
            pass
        # Comment out next 4 lines to avoid MySQLdb to simply read stream at consolew
        curr.execute(u"""INSERT INTO TwitterFeed (ID, Date, Lat, Lng, Text, Status) VALUES
                                    (%s, %s, %s, %s, %s, %s);""",
                    (status.id_str, status.created_at, XY[1], XY[0], text, str(status)))
        db.commit()
...
def main():
    ...
    while True:
        try:
            ...
            # stream.filter(track=['uber, uberdriver, uberx'])## This will feed the stream all mentions of 'keyword'
            break
        except Exception, e:
            print e
